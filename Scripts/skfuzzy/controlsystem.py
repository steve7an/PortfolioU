...
class ControlSystemSimulation(object):
    ...
    def get_antecedents(self):
        ''' Added this function to retrieve the antecendents in use'''
        dict_ant = {}
        for v in self.ctrl.antecedents:
            #print("{0:<35} = {1}".format(str(v), str(v.input[self])))
            for term in v.terms.values():
                #print("  - {0:<32}: {1}".format(term.label,term.membership_value[self]))
                if not term.membership_value[self] is None:
                    dict_ant[term.label] = term.membership_value[self]
        return dict_ant
    ...
    def print_state(self):
        ...
        for rn, r in enumerate(self.ctrl.rules):
            ...
            print("    {0:>54} = {1}".format(str(r.antecedent),str(r.aggregate_firing[self])))

            print("  Activation (THEN-clause):")
            for c in r.consequent:
                assert isinstance(c, WeightedTerm)
                print("    {0:>54} : {1}".format(str(c),
                                                 str(c.activation[self])))
            print("")
        print("")
